exports.BattleScripts = {
	...
	randomMagikTeam: function(side) {
		var team = [];
		for (var i=0; i<6; i++) {
			var template = Object.clone(this.getTemplate('magikarp'));
			template.moves = ['Splash', 'Tackle', 'Flail', 'Bounce'];
			if (Math.floor(Math.random() * 2) === 0) {
				template.ability = 'Swift Swim';
			}
			else {
				template.ability = 'Rattled';
			}
			template.evs = {
				hp: Math.floor(Math.random() * 86),
				atk: Math.floor(Math.random() * 86),
				def: Math.floor(Math.random() * 86),
				spa: Math.floor(Math.random() * 86),
				spd: Math.floor(Math.random() * 86),
				spe: Math.floor(Math.random() * 86)
			};
			template.ivs = {
				hp: Math.floor(Math.random() * 32),
				atk: Math.floor(Math.random() * 32),
			...
			template.level = Math.floor(Math.random() * 100) + 1;
			template.happiness = Math.floor(Math.random() * 256);
			template.shiny = false;
			template.item = 'Stick';
			team.push(template);
		}
		return team;
	},
	randomMagikMoveTeam: function(side) {
		var boostMoves = ["Acupressure", "Agility", "Amnesia",  "Aromatherapy", "Autotomize",
			"Barrier", "Belly Drum", "Bulk Up", "Calm Mind", "Camouflage", "Cosmic Power", "Cotton Guard", "Curse",
			"Defend Order", "Double Team", "Dragon Dance", "Focus Energy", "Gravity", "Heal Bell", "Healing Wish",
			"Hone Claws", "Growth", "Hail", "Ingrain", "Iron Defense", "Light Screen", "Lucky Chant", "Lunar Dance",
			"Magic Room", "Magnet Rise", "Milk Drink", "Minimize", "Moonlight", "Morning Sun", "Nasty Plot",
			"Quiver Dance", "Rain Dance", "Recover", "Reflect", "Refresh", "Rock Polish", "Roost", "Safeguard", "Sandstorm", "Shell Smash",
			"Shift Gear", "Skill Swap", "Slack Off", "Softboiled", "Substitute", "Sunny Day", "Swords Dance", "Synthesis",
			"Tail Glow", "Tailwind", "Transform", "Trick Room", "Wish", "Work Up"];
		var statusMoves = ["Attract", "Assist", "Block", "Captivate", "Charm", "Confuse Ray", "Copycat", "Cotton Spore", "Counter",
			"Destiny Bond", "Detect", "Disable", "Embargo", "Encore", "Endure", "Entrainment",
			"Fake Tears", "Featherdance", "Final Gambit", "Flatter", "Glare", "GrassWhistle",
			"Heal Block", "Imprison", "Leech Seed", "Lovely Kiss", "Magic Coat", "Mean Look", "Memento", "Metal Burst",
			"Metal Sound", "Metronome", "Mimic", "Mirror Coat", "Mirror Move", "Perish Song", "Protect", "Poisonpowder",
			"Snatch", "Scary Face", "Screech", "Sing", "Sketch", "Sleep Powder", "Snatch", "Soak", "Spite",
			"Stun Spore", "Supersonic", "Swagger", "Sweet Kiss", "Switcheroo", "Taunt", "Telekinesis", "Tickle", "Torment",
			"Toxic", "Trick", "Wonder Room", "Worry Seed"];
		var damageMoves = ["Aeroblast", "AncientPower", "Astonish", "Attack Order",
			"Blast Burn", "Blizzard", "Blue Flare", "Bolt Strike", "Bounce", "Brave Bird", "Bulldoze",
			"Close Combat", "Dig", "Dive", "Doom Desire", "Double-Edge", "Draco Meteor",
			"Earthquake", "Egg Bomb", "Eruption", "Explosion", "Extremespeed",
			"False Swipe", "Fire Blast", "Flail", "Flare Blitz", "Fly", "Focus Blast", "Focus Punch",
			"Freeze Shock", "Frenzy Plant", "Frustration", "Fusion Bolt", "Fusion Flare", "Giga Impact",
			"Head Smash", "Hurricane", "Hydro Cannon", "Ice Burn", "Judgement", "Leaf Storm", "Lick",
			"Magma Storm", "Magnitude", "Nature Power", "Night Shade", "Octazooka", "Ominous Wind", "Outrage", "Overheat",
			"Petal Dance", "Power Whip", "Present", "Psycho Boost", "Psystrike", "Return", "Revenge", "Roar of Time",
			"Sacred Fire", "Sacred Sword", "Searing Shot", "Secret Power", "Secret Sword", "Seed Flare", "Seismic Toss",
			"Selfdestruct", "Shadow Force", "Silver Wind", "Skull Bash", "Sky Attack", "Solarbeam", "Spacial Rend", "Super Fang", "Superpower",
			"Tackle", "Techno Blast", "Thrash", "Thunder", "U-turn", "V-create", "Volt Switch", "Volt Tackle", "Water Spout", "Wood Hammer"];
		var abilities = ["Adaptability", "Aftermath", "Anger Point", "Anticipation", "Bad Dreams", "Clear Body",
			"Color Change", "Compoundeyes", "Contrary", "Cursed Body", "Cute Charm", "Defiant", "Download", "Dry Skin", "Effect Spore",
			"Filter", "Flame Body", "Flash Fire", "Forewarn", "Frisk", "Guts", "Heatproof", "Huge Power", "Immunity",
			"Impostor", "Infiltrator", "Inner Focus", "Insomnia", "Intimidate", "Iron Barbs", "Justified", "Levitate",
			"Lightningrod", "Limber", "Magic Bounce", "Magic Guard", "Magma Armor", "Marvel Scale", "Mold Breaker",
			"Moody", "Motor Drive", "Moxie", "Multiscale", "Mummy", "Natural Cure", "No Guard", "Normalize", "Oblivious",
			"Own Tempo", "Poison Heal", "Poison Point", "Poison Touch", "Prankster", "Pressure", "Pure Power", "Quick Feet",
			"Rattled", "Regenerator", "Sap Sipper", "Serene Grace", "Shield Duse", "Simple", "Solid Rock", "Soundproof",
			"Speed Boost", "Static", "Stench", "Sticky Hold", "Storm Drain", "Sturdy", "Swift Swim", "Tangled Feet",
			"Technician", "Teravolt", "Thick Fat", "Tinted Lens", "Trace", "Turboblaze", "Unaware", "Victory Star",
			"Vital Spirit", "Volt Absorb", "Water Absorb", "Weak Armor", "White Smoke", "Wonder Skin"];
		var items = ["Apicot Berry", "Aspear Berry", "Berry Juice", "BrightPowder", "Cheri Berry", "Chesto Berry",
			"Choice Band", "Choice Scarf", "Choice Specs", "Custap Berry", "Destiny Knot", "Eject Button",
			"Enigma Berry", "Eviolite", "Expert Belt", "Focus Band", "Focus Sash", "Ganlon Berry", "Jaboca Berry",
			"King's Rock", "Lansat Berry", "Leftovers", "Leppa Berry", "Liechi Berry", "Life Orb", "Lum Berry",
			"Mental Herb", "Metronome", "Micle Berry", "Muscle Band", "Oran Berry", "Pecha Berry", "Persim Berry",
			"Petaya Berry", "Quick Claw", "Rawst Berry", "Razor Claw", "Razor Fang", "Red Card", "Rocky Helmet",
			"Rowap Berry", "Salac Berry", "Scope Lens", "Shed Shell", "Shell Bell", "Sitrus Berry", "Starf Berry",
			"Stick", "White Herb", "Wide Lens", "Wise Glasses", "Zoom Lens"];

		var team = [];
		for (var i=0; i<6; i++) {
			boostMoves = boostMoves.randomize();
			statusMoves = statusMoves.randomize();
			damageMoves = damageMoves.randomize();
			abilities = abilities.randomize();
			items = items.randomize();
			var template = Object.clone(this.getTemplate('magikarp'));
			template.moves = ['Splash', boostMoves[0], statusMoves[0], damageMoves[0]];
			template.ability = abilities[0];
			template.evs = {
				hp: Math.floor(Math.random() * 86),
				atk: Math.floor(Math.random() * 86),
				def: Math.floor(Math.random() * 86),
			...
			template.level = 100;
			template.happiness = Math.floor(Math.random() * 256);
			template.shiny = Math.floor(Math.random() * 1024) < 1;
