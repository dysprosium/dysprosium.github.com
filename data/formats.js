exports.BattleFormats = {
	...
	monotypeou: {
		...
		searchShow: true,
		isTeambuilderFormat: true,
		ruleset: ['Pokemon', 'Standard', 'Team Preview', 'Monotype'],
	...
	randommagikbattle: {
		effectType: 'Format',
		name: "Random Magik Battle",
		section: 'special',
		team: 'randomMagikMove',
		canUseRandomTeam: true,
		challengeShow: true,
		searchShow: true,
		ruleset: [],
		...
		validateTeam: function(team, format) {
			...
			for (var i=0; i<team.length; i++) {
				...
				if (i == 0) {
					...
					for (var j=0; j<teamType.length; j++) {
						for (var k=0; k<types.length; k++) {
							if (types[k] == teamType[j]){
								typeCheck[j] = true;
							}
						}
					}
					if(!typeCheck[0] && !typeCheck[1]){
						return ["All of your pokemon must be the same type."];
					}
					if(typeCheck[0] && !typeCheck[1]){
						teamType = [teamType[0]];
					}
